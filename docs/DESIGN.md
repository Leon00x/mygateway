...
两页复用页面标题、Usage / Logs 分段切换和筛选控件样式，延续现有浅色/暗黑主题、紫色强调、
16–20px 面板圆角和移动端单列规则，不引入新的组件库。旧 `/requests` 保留兼容跳转。
...
- 首屏参考 QwenCloud：左侧高卡展示总 Token、输入/输出构成和预估费用；右侧 2×2 展示请求量、
  平均延迟、平均流式 TTFT 和成功率。无可信 Token 或 TTFT 时明确显示未知覆盖率，不显示假 0。
- 下方模型表展示模型、平均 TPM/RPM、请求、成功率、平均 TTFT、平均延迟、Token 与费用；
