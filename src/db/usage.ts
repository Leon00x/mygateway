 ...
 */
export function getUsageRange(range: 'today' | '7d' | '30d'): { start: number; end: number } {
  const now = Date.now();
  ...
  if (range === 'today') {
    ...
    // Simplified: use UTC midnight as boundary for now
    const todayStart = new Date(nowDate);
    todayStart.setUTCHours(0, 0, 0, 0);
    startMinute = Math.floor(todayStart.getTime() / 60_000) * 60;
  } else if (range === '7d') {
