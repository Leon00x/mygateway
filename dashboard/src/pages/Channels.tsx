  ...
  return <div class="resource-page channel-page">
    ...
    <Show when={!loading()}><div class="channel-card-grid">
      ...
      return <article class="panel channel-card">
        <header class="channel-card-head"><ProviderLogo presetId={channel.preset_id} name={channel.name} /><div><strong>{channel.name}</strong><span>{providerTypeLabel(channel.provider_type)}</span></div><span class={`badge ${channel.status}`}>{channel.status === 'active' ? t('common.active') : t('common.disabled')}</span></header>
