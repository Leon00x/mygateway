  ...
  return <div class="resource-page channel-page">
    ...
    <div class="channel-card-grid"><For each={filteredChannels()}>{(channel) => {
      ...
      return <article class="panel channel-card">
        <header class="channel-card-head"><span class="provider-logo">{channel.name.slice(0, 1).toUpperCase()}</span><div><strong>{channel.name}</strong><span>{channel.provider_type.replace('_', ' ')}</span></div><span class={`badge ${channel.status}`}>{channel.status === 'active' ? '运行中' : '已停用'}</span></header>
